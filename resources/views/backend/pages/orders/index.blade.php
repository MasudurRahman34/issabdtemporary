      ...
      <div class="card-body">
        ...
        <table class="table table-hover table-striped" id="dataTable">
          ...
          <tbody>
            ...
            <tr>
              <td>{{ $loop->index + 1 }}</td>
              <td>{{ $order->order_gen_id }}</td>
              <td>{{ $order->name }}</td>
              <td>{{ $order->phone_no }}</td>
              <td>{{ \Carbon\Carbon::parse($order->created_at)->diffForHumans(now()) }}<br/>{{date('Y-m-d', strtotime($order->created_at))}}</td>
              <td>
                {{-- <p>
              ...
              <td>
                ...
                <a href="{{ route('admin.order.show', $order->id) }}"  class="btn btn-info">দেখুন </a>
